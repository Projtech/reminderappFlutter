...
class _AuthScreenState extends State<AuthScreen> {
 ...
 void _startLockoutTimer() {
   if (_isInLockout && _lockoutMinutes > 0) {
     _lockoutTimer = Timer.periodic(const Duration(seconds: 30), (timer) async {
 ...
 Future<void> _authenticateWithBiometric() async {
   ...
   if (success && mounted) {
     ...
     // Se falhou e só tem biometria, mostrar erro
 ...
 Future<void> _authenticateWithPin() async {
   ...
   if (success && mounted) {
     ...
     if (_isInLockout) {
       _showMessage('Muitas tentativas! Aguarde $_lockoutMinutes minutos.', Colors.red);
     } else {
       _showMessage('PIN incorreto. Tentativas restantes: ${3 - _failedAttempts}', Colors.red);
     }
   }
 }
           ...
           children: [
             ...
             Text(
               ...
               style: theme.textTheme.bodyLarge?.copyWith(
                 color: _isInLockout ? Colors.red : theme.textTheme.bodyLarge?.color,
               ),
                 ...
                 child: GridView.builder(
                   ...
                   itemBuilder: (context, index) {
                     if (index == 9) {
                       ...
                       // Botões 1-9
                       return _buildNumberButton((index + 1).toString(), theme);
                     }
                   },
                 ),
               ),
             ...
             if (_failedAttempts > 0 && !_isInLockout)
               Text(
                 'Tentativas restantes: ${3 - _failedAttempts}',
